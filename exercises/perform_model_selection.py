...
def select_polynomial_degree(n_samples: int = 100, noise: float = 5):
    ...
    fig2 = go.Figure(
        data=[
            ...
            go.Scatter(x=[best_k], y=[selected_error], mode='markers',
                       name='Selected Model', marker=dict(color='darkred', symbol="x", size=10))
        ],
        layout=go.Layout(
            title=fr'5-fold cross validation',
            xaxis_title='k',
    ...
    print(f"test error for k* = {best_k} : {reported_error}")
...
def select_regularization_parameter(n_samples: int = 50, n_evaluations: int = 500):
    ...
    train_samples = 50
    train_X, train_y, test_X, test_y = X[:train_samples], y[:train_samples], \
    ...
    fig3 = make_subplots(rows=2,
                         cols=1,
                         subplot_titles=[f"train and validation errors as a function of the tested regularization parameter value: {m}" for m in
                                         models],
...
if __name__ == '__main__':
    ...
    select_polynomial_degree(n_samples=1500, noise=10)  # 4
